...
Chi = np.copy(data)
data[:, [1,2]] = data[:, [1,2]] * 7.11e20
data[:, [1,2]] = np.divide(data[:, [1,2]], 1 - data[:, [1,2]] / 3.0)
...
plt.title("Real Part of the Susceptibility")
plt.ylabel(r'$\chi^{\prime}$')
...
plt.title("Imaginary Part of the Susceptibility")
plt.ylabel(r'$\chi^{\prime\prime}$')
plt.xlabel("Frequency (Hz)")
plt.xscale('log')
#plt.yscale('log')
#plt.tight_layout()
